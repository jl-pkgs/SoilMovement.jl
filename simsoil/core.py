...
class InfiltrationModel(object):
    ...
    def run(
        ...
        if n_days is None:
            n_days = influx.size
        est_vwc = np.ones((self.soil.nlayer, n_days)) * np.nan
        est_error = np.ones((self.soil.nlayer, n_days)) * np.nan
        est_psi = None
        if self._debug:
            est_psi = np.ones((self.soil.nlayer, n_days)) * np.nan
            if not climatology:
...
class SoilProfile(object):
    ...
    def __init__(
        ...
        self.nlayer = self._depths_m.size
